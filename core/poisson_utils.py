...
class PoissonModel:
    ...
    def __init__(
        ...
        Args:
            home_advantage: Множител за предимство на домакина (deprecated - използва се per-league)
            league_normalization: Дали да нормализираме по лига
            min_matches: Минимален брой мачове за изчисляване на strength
            shrinkage_alpha: Shrinkage коефициент към лигови средни
        """
        self.home_advantage = home_advantage  # Fallback
        ...
        self.league_home_advantage: Dict[int, float] = {
            # Default values per league type
            0: 1.08,    # Default/Unknown
        ...
        self.logger.info("PoissonModel инициализиран с подобрения")
    ...
    def calculate_lambda(
        ...
        Args:
            ...
            home_form_5: Форма на домакина (0-1, default 0.5)
            away_form_5: Форма на гостите (0-1, default 0.5)
        ...
        if league_id and league_id in self.league_avg_goals_home:
            ...
            league_attack_avg = (avg_home + avg_away) / 2
            league_defense_avg = league_attack_avg  # Symmetric assumption
        else:
        ...
        # Team strengths with shrinkage
        home_attack_raw = self.attack_strength.get(home_team_id, 1.0)
        home_defense_raw = self.defense_strength.get(home_team_id, 1.0)
        away_attack_raw = self.attack_strength.get(away_team_id, 1.0)
        away_defense_raw = self.defense_strength.get(away_team_id, 1.0)
        
        # Apply shrinkage towards league averages
        alpha = self.shrinkage_alpha
        home_attack = np.clip((1 - alpha) * home_attack_raw + alpha * league_attack_avg, 0.6, 2.5)
        home_defense = np.clip((1 - alpha) * home_defense_raw + alpha * league_defense_avg, 0.6, 2.5)
        away_attack = np.clip((1 - alpha) * away_attack_raw + alpha * league_attack_avg, 0.6, 2.5)
        away_defense = np.clip((1 - alpha) * away_defense_raw + alpha * league_defense_avg, 0.6, 2.5)
        ...
        # Per-league home advantage
        home_advantage = self.league_home_advantage.get(league_id, self.home_advantage)
        
        # Calculate λ with form adjustments
        lambda_home = avg_home * home_attack * away_defense * home_advantage * form_adj_home
        lambda_away = avg_away * away_attack * home_defense * form_adj_away
    ...
    def predict_match_probabilities(
        ...
        max_goals = max(8, int(np.ceil(lambda_home + lambda_away + 3)))
